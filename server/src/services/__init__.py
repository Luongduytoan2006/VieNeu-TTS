...
* ``create_audio`` — orchestrator API tạo TTS: check điều kiện + rẽ nhánh CPU/GPU.
* ``cpu_onnx``    — chạy synth in-process trên máy này (CPU/ONNX).
* ``gpu_vastai``  — provision Vast.ai on-demand (tạo→chạy→upload→destroy).
* ``synth_job``   — script chạy TRÊN máy GPU Vast.ai (batch engine tác giả).
