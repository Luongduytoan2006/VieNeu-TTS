...
Sync on purpose: synthesis runs in worker threads (the model isn't async), so a
sync ORM is the simplest correct fit. Each unit of work opens a short-lived
Session via the ``session_scope`` context manager.
"""
